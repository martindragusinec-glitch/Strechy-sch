...
- **Kalkulačka nároku `#narok`:** roční fakturace → zisk po paušálních výdajích **70 %** (průměr 60/80 %, zadání 21. 9.) − daň 15 % po slevě 30 840 − minimální pojistné 2026 (soc. 5 720, zdr. 3 306 Kč/měs.) = čistý příjem; + partner, děti do 13 (0,3), osoby 14+ (0,5); přepínače důchodci / ID3 (5. decil) / superdávka (nárok vždy); stav domu (E–G). Verdikt ok / na hraně (do +15 %) / ne → bezúročný úvěr. Výsledek jde do leadu (`note_fields['Kalkulačka nároku OSVČ']`, `extra.elig_calc`) a při kliku na CTA se pushne `eligibility_calc` do dataLayer.
